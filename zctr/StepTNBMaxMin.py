...
# Min-max normalization (Rescaling) 使用的是几率
# https://zhuanlan.zhihu.com/p/624115618

class StepTNBMaxMin:

    @staticmethod
    def subTask1(processId, tasks, datadir, MAX, ie, be, PC, NC):
    ...
    @staticmethod
    def subTask2(tasks, datadir, MAX, be, ie, CH):
        ...
        for taskid, subus in tasks:
            ...
            IOKit.write_pkl("Data/" + datadir + "/RNBMaxMin_"+str(taskid)+".pkl", R)
    ...
    def execute(dataset, top=None):
        ...
        for i in range(process_total):
            p = multiprocessing.Process(
                target=StepTNBMaxMin.subTask1, args=(i, tasks[i], dataset, MAX, ie, be, PC, NC))
            ps.append(p)
        ...
        PC = (PC) / np.sum(PC)
        NC = (NC) / np.sum(NC)
        CH = [PC[i] / (NC[i] + 0.00000001) for i in range(be.get_bs_size())]

        ps = []
        for i in range(process_total):
            p = multiprocessing.Process(
                target=StepTNBMaxMin.subTask2, args=(tasks[i], dataset, MAX, be, ie, CH))
            ps.append(p)
        ...
        R = []
        for i in range(batch):
            data = IOKit.read_pkl("Data/" + dataset + "/RNBMaxMin_"+str(i)+".pkl")
            for d in data:
                R.append(d)
        ...
        log = open("Data/"+ dataset +"/Z_RNBMaxMin.txt", mode='w')
